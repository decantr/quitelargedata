...
\pagebreak\subsubsection{Distribution}
<<>>=
...
<<>>=
plot(forest, main = "Error rate for first model")
@
...
\pagebreak We can see it does indeed match 
<<>>=
plot(forSmall, main = "Error rate for second iteration")
@
