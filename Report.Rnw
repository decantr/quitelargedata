...
forSmall <- randomForest(as.factor(class) ~ 
	odor + stalk.root + spore.print.color + gill.size + gill.color +
	habitat + stalk.shape + ring.number + population, data = train, importance = T)

print(forSmall) # print out the forest so we can see the accuracy
preSmall <- predict(forSmall, test) # use the forest in a prediction
table(preSmall == test$class) # check if we got it right
@
